// Bundled code from https://github.com/TorBox-App/parse-torrent-title
...
var require_transformers = __commonJS({
  "src/transformers.js"(exports2) {
    var moment = require_moment();
    exports2.none = (input) => input;
    exports2.value = (value) => (input) => {
      if (typeof value === "string") {
        return value.replace("$1", input);
      }
      return value;
    };
    exports2.integer = (input) => parseInt(input, 10);
    exports2.boolean = () => true;
    exports2.lowercase = (input) => input.toLowerCase();
    exports2.uppercase = (input) => input.toUpperCase();
    exports2.date = (dateFormat) => (input) => {
      const sanitized = input.replace(/\W+/g, " ").trim();
      const date = moment(sanitized, dateFormat);
      return date.format("YYYY-MM-DD");
    };
    exports2.range = (input) => {
      let array = input.replace(/\D+/g, " ").trim().split(" ").map((str) => parseInt(str, 10));
    ...
    exports2.array = (chain) => (input) => [chain ? chain(input) : input];
    exports2.uniqConcat = (chain) => (input, result) => {
...
var require_parser = __commonJS({
  "src/parser.js"(exports2) {
    var { none } = require_transformers();
    var NON_ENGLISH_CHARS = "\u3040-\u30FF\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF\uFF66-\uFF9F\u0400-\u04FF";
    var RUSSIAN_CAST_REGEX = new RegExp("\\([^)]*[\u0400-\u04FF][^)]*\\)$|(?<=\\/.*)\\(.*\\)$");
    var ALT_TITLES_REGEX = new RegExp(`[^/|(]*[${NON_ENGLISH_CHARS}][^/|]*[/|]|[/|][^/|(]*[${NON_ENGLISH_CHARS}][^/|]*`, "g");
    var NOT_ONLY_NON_ENGLISH_REGEX = new RegExp(`(?<=[a-zA-Z][^${NON_ENGLISH_CHARS}]+)[${NON_ENGLISH_CHARS}].*[${NON_ENGLISH_CHARS}]|[${NON_ENGLISH_CHARS}].*[${NON_ENGLISH_CHARS}](?=[^${NON_ENGLISH_CHARS}]+[a-zA-Z])`, "g");
    var NOT_ALLOWED_SYMBOLS_AT_START_AND_END = new RegExp(`^[^\\w${NON_ENGLISH_CHARS}#[\u3010\u2605]+|[ \\-:/\\\\[|{(#$&^]+$`, "g");
    var REMAINING_NOT_ALLOWED_SYMBOLS_AT_START_AND_END = new RegExp(`^[^\\w${NON_ENGLISH_CHARS}#]+|[[\\]({} ]+$`, "g");
    var DEFAULT_OPTIONS = {
    ...
    function createHandlerFromRegExp(name, regExp, transformer, options) {
      function handler({ title, result, matched }) {
        ...
        if (rawMatch) {
          ...
          const isSkipIfFirst = options.skipIfFirst && otherMatches.length && otherMatches.every((e) => match.index < e[1].matchIndex);
    ...
    function cleanTitle(rawTitle) {
      ...
      cleanedTitle = cleanedTitle.replace(/_/g, " ").replace(/[[(]movie[)\]]/i, "").replace(NOT_ALLOWED_SYMBOLS_AT_START_AND_END, "").replace(RUSSIAN_CAST_REGEX, "").replace(/^[[【★].*[\]】★][ .]?(.+)/, "$1").replace(/(.+)[ .]?[[【★].*[\]】★]$/, "$1").replace(ALT_TITLES_REGEX, "").replace(NOT_ONLY_NON_ENGLISH_REGEX, "").replace(REMAINING_NOT_ALLOWED_SYMBOLS_AT_START_AND_END, "").trim();
      return cleanedTitle;
    }
    var Parser2 = class {
      constructor() {
...
var require_handlers = __commonJS({
  "src/handlers.js"(exports2) {
    var { none, value, integer, boolean, lowercase, uppercase, date, range, yearRange, array, uniqConcat } = require_transformers();
    exports2.addDefaults = /** @type Parser */
    (parser) => {
      ...
      parser.addHandler("episodes", ({ title, result, matched }) => {
        if (!result.episodes) {
          const startIndexes = [matched.year, matched.seasons].filter((component) => component).map((component) => component.matchIndex).filter((index) => index > 0);
          const endIndexes = [matched.resolution, matched.source, matched.codec, matched.audio].filter((component) => component).map((component) => component.matchIndex).filter((index) => index > 0);
          ...
          const matches = Array.from(beginningTitle.matchAll(/(?<!movie\W*|film\W*|^)(?:[ .]+-[ .]+|[([][ .]*)(?!(?:5|7)\.1\b)(\d{1,4})(?:a|b|v\d|\.\d)?(?:\W|$)(?!movie|film|\d+)/gi)).pop() || middleTitle.match(/^(?:[([-][ .]?)?(\d{1,4})(?:a|b|v\d)?(?!\Wmovie|\Wfilm|-\d)(?:\W|$)/i);
          if (matches) {
            result.episodes = [matches[matches.length - 1]].map((group) => group.replace(/\D/g, "")).map((group) => parseInt(group, 10));
      ...
      parser.addHandler("languages", ({ title, result, matched }) => {
        if (!result.languages || ["portuguese", "spanish"].every((l) => !result.languages.includes(l))) {
          if (matched.episodes && matched.episodes.rawMatch.match(/capitulo|ao/i) || title.match(/dublado/i)) {
      ...
      parser.addHandler("group", ({ result, matched }) => {
        if (matched.group && matched.group.rawMatch.match(/^\[.+]$/)) {
          ...
          if (Object.keys(matched).some((key) => matched[key].matchIndex && matched[key].matchIndex < endIndex)) {
            delete result.group;
          }
        }
        return { matchIndex: 0 };
...
var trasformers = require_transformers();
